...
async def test_all(dut):
    ...
    await with_timeout(RisingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.rom_loader_load), 1000, 'us')


    count = 0
    print("Loading rom: 0/16 instructions loaded\r", end='\r')
    while(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.rom_loader_load==1):
        trigger_sck = FallingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.rom_loader_sck)
        trigger_load = FallingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.rom_loader_load)
    ...
    # await with_timeout(RisingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.rom_loader_load), 2000, 'us')


    print("Waiting reset of the Hack cpu...")
    await with_timeout(FallingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.hack_reset), 350, 'us')
    print("Hack soc reset done")


    for i in range(0,24):
        await ClockCycles(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.hack_clk, 1)
        print("PC: ", int(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.hack_pc.value), " INSTRUCTION:", hex(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.hack_instruction.value))
        if(i==4):
    ...
    await ClockCycles(dut.uut.mprj.wrapped_hack_soc_dffram_8.soc.hack_clk, 5)
