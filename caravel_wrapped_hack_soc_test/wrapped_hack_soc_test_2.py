...
async def test_start(dut):
    ...
    await RisingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_11.active)
...
async def test_all(dut):
    ...
    while(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.rom_loader_load.value.binstr!='1'):
        await ClockCycles(dut.clock, 1)
        print("Waiting for the rom loader to start...  [ rom_loader_load =", dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.rom_loader_load.value.binstr, "  la1_data_in =", dut.uut.mprj.wrapped_hack_soc_dffram_11.la1_data_in.value.binstr, "]",  end='\r')

    print("")
    # await with_timeout(RisingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.rom_loader_load), 1200, 'us')


    count = 0
    print("Loading rom: 0/16 instructions loaded\r", end='\r')
    while(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.rom_loader_load==1):
        # print("Loading rom: ", count, "/16 instructions loaded   [ la1_data_in =", dut.uut.mprj.wrapped_hack_soc_dffram_11.la1_data_in.value.binstr, " ]", end='\r')
        trigger_sck = FallingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.rom_loader_sck)
        trigger_load = FallingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.rom_loader_load)
    ...
    # await with_timeout(RisingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.rom_loader_load), 2000, 'us')


    print("Waiting reset of the Hack cpu...")
    await with_timeout(FallingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.hack_reset), 350, 'us')
    print("Hack soc reset done")


    for i in range(0,24):
        await ClockCycles(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.hack_clk, 1)
        print("PC: ", int(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.hack_pc.value), " INSTRUCTION:", hex(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.hack_instruction.value))
        if(i==4):
            # await FallingEdge(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.spi_video_ram_1.display_active)
    ...
    await ClockCycles(dut.uut.mprj.wrapped_hack_soc_dffram_11.soc.hack_clk, 5)
...
async def test_all_gl(dut):
    ...
    count = 0
    while(dut.uut.mprj.wrapped_hack_soc_dffram_11.la1_data_in[10].value.binstr!='1' and count < 8000):
        count = count + 1
    ...
    sck_was_high = 1
    print("Loading rom: 0/16 instructions loaded\r", end='\r')
    while(dut.uut.mprj.wrapped_hack_soc_dffram_11.la1_data_in[10].value.binstr=='1' and count < 40000):
        count = count + 1            
        print("Loading rom: ", count_sck, "/16 instructions loaded   [ ", count, "cycles (timeout at 40000) ]", end='\r')        
        if(sck_was_high and dut.uut.mprj.wrapped_hack_soc_dffram_11.la1_data_in[9].value.binstr=='0'):
            count_sck = count_sck + 1
            sck_was_high = 0
            # print("Loading rom: ", count_sck, "/16 instructions loaded   [ ", count, "cycles (timeout at 40000) ]", end='\r')        
        if(dut.uut.mprj.wrapped_hack_soc_dffram_11.la1_data_in[9].value.binstr=='1'):
            sck_was_high = 1
        await(ClockCycles(dut.clock, 1))
